...
import numpy as np
with open('scatter.csv') as csvfile:
...
ax=fig.add_subplot()
ax=plt.gca()
plt.title('Day View (F1-score)')
plt.grid(b=True, axis='y',alpha=0.3,linewidth=1.2)
plt.scatter(np.array(x2)*2,np.array(y1),color='tomato')
plt.scatter(np.array(x2)*2+0.95,np.array(y2),color='limegreen')
plt.bar(np.arange(len(x1))*2,np.array(e1),width=0.95,alpha=0.2,color='coral')
plt.bar(np.arange(len(x1))*2+0.95,np.array(e2),width=0.95,alpha=0.2,color='palegreen')
