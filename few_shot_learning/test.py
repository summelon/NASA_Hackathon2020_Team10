...
if __name__ == '__main__':
  ...
  datamgr = SetDataManager(image_size, n_eposide = iter_num, n_query = 70 , **few_shot_params, night=params.night)
  loadfile = os.path.join(params.data_dir, params.dataset, split + '.json')#configs.data_dir[params.dataset] + split + '.json'
  novel_loader     = datamgr.get_data_loader( loadfile, aug = False)

  # testing
  model.eval()
  acc_mean, acc_std = model.test_loop(novel_loader, return_std = True, cls_info=True)
