...
class MetaTemplate(nn.Module):
    ...
    def correct(self, x, cls_info=False):
        ...
        if cls_info:
            correct_counter = {cls: 0 for cls in range(self.n_way)}
            for lbl, pred in zip(y_query, topk_ind[:, 0]):
                if lbl == pred:
                    correct_counter[lbl] += 1
            return float(top1_correct), len(y_query), loss.item()*len(y_query), y_query, topk_ind[:, 0]
    ...
    def test_loop(self, epoch, test_loader):
        #correct =0
        loss = 0.
        count = 0
        acc_all = []
    ...
    def set_scheduler(self, cur_ep, stop_ep):
      return
